...
def load_mapping(root_mapping, pathrow=None, strip_id=None, list_p=None):
    """
    ...
    pathrow: str (optional), default None
        pathrow that you want.
    strip_id: str (optional), default None
        strip_id that you want.
    list_p: list of str of province (optional), default None
        list of p that you want.    
    
    Examples
    --------
    >>> load_mapping(root_mapping, pathrow="129049")
    >>> load_mapping(root_mapping, strip_id="305")
    >>> load_mapping(root_mapping, strip_id="305", list_p=["32", "33"])
    
    Returns
    -------
    Concatenated mapping dataframe, list_p
    """
    fn_p_from_file = lambda file: os.path.splitext(file)[0].split("_")[-2][1:]
    fn_pathrow_from_file = lambda file: os.path.splitext(file)[0].split("_")[-1]
    fn_strip_id_from_file = lambda file: os.path.splitext(file)[0].split("_")[-1][1:]
    
    # Pathrow
    if pathrow is not None:
        if list_p is None:
            list_p = [fn_p_from_file(file) for file in os.listdir(root_mapping) if fn_pathrow_from_file(file) == pathrow]
            df = pd.concat([pd.read_parquet(os.path.join(root_mapping, file)) for file in os.listdir(root_mapping) if fn_pathrow_from_file(file) == pathrow], ignore_index=True)
        else:
            df = pd.concat([pd.read_parquet(os.path.join(root_mapping, file)) for file in os.listdir(root_mapping) if (fn_pathrow_from_file(file) == pathrow) and fn_p_from_file(file) in list_p], ignore_index=True)
    
    # Strip_id
    elif strip_id is not None:
        if list_p is None:
            list_p = [fn_p_from_file(file) for file in os.listdir(root_mapping) if fn_strip_id_from_file(file) == strip_id]
            df = pd.concat([pd.read_parquet(os.path.join(root_mapping, file)) for file in os.listdir(root_mapping) if fn_strip_id_from_file(file) == strip_id], ignore_index=True)
        else:
            df = pd.concat([pd.read_parquet(os.path.join(root_mapping, file)) for file in os.listdir(root_mapping) if (fn_strip_id_from_file(file) == strip_id) and fn_p_from_file(file) in list_p], ignore_index=True)
    return df, list_p
